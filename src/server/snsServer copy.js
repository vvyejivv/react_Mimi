...
app.get('/postListAll.dox', function (req, res) {
    ...
    GROUP BY  P.POSTNO , U.USERID, U.NAME, INTRO, TITLE, CONTENTS, LIKES, HIT, 
    (SELECT COUNT(POSTNO) FROM USER_POST), 
    DATE_FORMAT(P.CDATE, '%Y/%m/%d %p %h:%i'),
    H.FILENAME ,
    C.COMMENTNO, C.USERID, C.COMMENT,
	DATE_FORMAT(C.CDATE, '%Y/%m/%d %p %h:%i'),
    I.FILENAME    
...
// SELECT P.POSTNO AS POSTNO, U.USERID, U.NAME, INTRO, TITLE, CONTENTS, LIKES, HIT, 
// (SELECT COUNT(POSTNO) FROM USER_POST) AS POSTCNT, 
// DATE_FORMAT(P.CDATE, '%Y/%m/%d %p %h:%i') AS CDATE,
// H.FILENAME   AS PATH,
// C.COMMENTNO, C.USERID AS COMMENTID, C.COMMENT,
